...
> webhook management. The platform host serves the standalone developer console
> with password and Google sign-in, MFA step-up for sensitive mutations, and a
> deployment receipt at `/api/deploy-version`. Exact recorded browser-action
...
Partner-preview access is invitation-only. The developer console is served at
`https://platform.praxa.io` with sign-in at `/sign-in` and a deployment receipt
at `/api/deploy-version` identifying the application and its exact stamped
commit. Consumer-host developer deep links on `app.praxa.io` redirect to the
platform host. See [Studio and console](/fabric/studio-and-console) for the
per-surface truth table and the bounded evidence and remaining gates on the
[status page](/fabric/status-and-roadmap).
