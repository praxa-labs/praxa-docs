---
title: "Praxa CLI Command Reference"
sidebarTitle: "Commands"
description: "Full reference for all praxa CLI commands: version, doctor, mission get, mission create, and mission cancel. Includes flags, environment variables, and example output."
---
...
Submits a new mission to the Praxa Integration Gateway by reading a JSON file from disk and posting it to the `/v8/missions` endpoint. The response contains the assigned run ID and the initial mission state.
...
Requests durable cancellation of a running mission through the Control Plane. The reason string is recorded in the mission audit log. You must provide a unique idempotency key so the cancellation request is safely retryable in the event of a network failure.
