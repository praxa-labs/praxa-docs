...
description: "A mission is a governed, goal-directed agent task in Praxa. Learn about mission structure, lifecycle states, steps, and how the Integration Gateway executes missions."
---
...
| `cancelled` | You (or your application) called `cancelMission()` and the cancellation was propagated by the Control Plane. |
...
Each emitted `AuraSseEvent` carries an `id` (cursor), an `event` type string, and a `data` payload parsed from JSON. If your connection drops mid-stream, pass the last received `id` as `lastEventId` when you reconnect — the gateway replays events from your cursor position within its bounded durable replay window.
