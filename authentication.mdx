---
title: "Authenticate with the Praxa Integration Gateway"
...
Pass your token through the `accessToken` option when constructing a `PraxaClient`. This option accepts either a string or a function that returns a string or a `Promise<string>`. Using an async function is strongly preferred in production because it lets your application refresh the token transparently without recreating the client.
...
<Tip>
  In production, always use an async token provider function rather than a static string. This lets you integrate with your platform's secret management or session layer and rotate tokens without restarting your application.
</Tip>
...
The older `AURA_BASE_URL` and `AURA_ACCESS_TOKEN` environment variables remain supported as compatibility fallbacks, but `PRAXA_BASE_URL` and `PRAXA_ACCESS_TOKEN` are preferred for new integrations.
