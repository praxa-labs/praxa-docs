...
keywords: ["Authenticate with the Praxa Integration Gateway API","Praxa developer platform","governed AI agents","Praxa documentation"]
---
...
**Local smoke test — environment-backed token:**

```ts
import { PraxaClient } from "@praxa/sdk";

const accessToken = process.env.PRAXA_ACCESS_TOKEN;
if (!accessToken) throw new Error("PRAXA_ACCESS_TOKEN is required");
...
|---|---|
| `missions:write` | Submit intent, create a mission, signal a mission, cancel a mission |
...
| `mcp:invoke` | MCP 2025-11-25 Streamable HTTP JSON-RPC endpoint with negotiated 2025-03-26 compatibility |
