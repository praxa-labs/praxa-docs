...
keywords: ["Praxa Integration Gateway REST API v8.1.0 Reference","Praxa API reference","Praxa Integration Gateway","OAuth API"]
---

The Praxa Integration Gateway exposes a REST API at version 8.1.0. Its stable wire contract remains `aura-integration-gateway-v8.1` for compatibility, including through Praxa-named SDK exports. Models, SDKs, CLIs, and MCP tools grant no action authority of their own — all effects are governed by the platform's policy and consent controls. This reference covers all 15 operations available in v8.1.0.
