---
title: "Praxa Integration Gateway REST API Reference"
sidebarTitle: "Overview"
description: "The Praxa Integration Gateway REST API (v8.1.0) lets you create, monitor, and control governed AI agent missions. All endpoints require OAuth Bearer authentication."
---

The Praxa Integration Gateway exposes a REST API at version 8.1.0 (contract: `aura-integration-gateway-v8.1`) for submitting and managing governed AI agent missions. The public edge delegates every governed operation to a private deterministic policy, broker, and verifier plane — models, SDKs, CLIs, and MCP tools grant no action authority of their own. This reference covers all 14 endpoints available in v8.1.0.
...
<Tip>
  The `/mcp` endpoint sits outside the `/v8/` prefix because it implements the
