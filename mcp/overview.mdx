...
- **`PRAXA_MCP_TOOLS`** — An array of 12 `PraxaMcpTool` objects. Each object contains the tool name, description, `inputSchema` (JSON Schema), `operationId`, HTTP method and path, required OAuth scope, and idempotency behavior.
...
- **`MCP_SERVER_NAME`** — The string `"praxa-agent-os"`, the canonical server identifier used in the hosted `/mcp` endpoint.
