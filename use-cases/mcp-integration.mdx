---
title: "Integrate Praxa with MCP-Compatible AI Agents"
sidebarTitle: "MCP Integration"
description: "Use Praxa's MCP 2025-03-26 contracts to integrate with any MCP-compatible AI agent framework. The @praxa/mcp-contracts package provides ready-to-use tool definitions."
---
...
Pass `MCP_PROTOCOL_VERSION` to your server's handshake or capability advertisement to declare compatibility with the MCP 2025-03-26 specification. The `MCP_SERVER_NAME` value is the canonical wire-level identity for the Praxa gateway — use it when configuring client discovery or logging.

<Note>
  `MCP_SERVER_NAME` is `"aura-agent-os"` — Aura is the wire-level compatibility
  identifier for the Praxa Integration Gateway. Both names refer to the same
  service; Praxa is the public product name and Aura is the stable
  contract-layer name.
</Note>
