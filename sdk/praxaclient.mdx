---
title: "PraxaClient: Full SDK Reference"
sidebarTitle: "PraxaClient"
description: "Complete reference for PraxaClient — the main class in @praxa/sdk. Covers all constructor options, every method signature, and examples for missions, capabilities, memory, traces, and skills."
---
...
<ParamField path="accessToken" type="string | (() => string | Promise<string>)" required>
  A short-lived delegated OAuth access token, or an async function that returns one. The SDK calls your provider before every request, so you can rotate tokens transparently. Tokens must be 1–16,384 characters and must not contain carriage-return or line-feed characters.
</ParamField>
