...
description: "The Praxa benchmarks suite measures mission latency, event streaming throughput, SDK retry performance, and Integration Gateway reliability under various workloads."
---
...
**Reproducibility constraints.** Because results depend on your gateway deployment, network path, and the complexity of each `goalSpec`, the benchmarks suite records the `PRAXA_BASE_URL`, SDK version (`PRAXA_OPENAPI_VERSION = "8.1.0"`), and contract fingerprint (`PRAXA_CONTRACT_VERSION = "aura-integration-gateway-v8.1"`) alongside every result file. Do not compare results across different contract versions or gateway deployments without accounting for those variables.
