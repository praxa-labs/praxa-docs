...
keywords: ["Praxa SDK performance characteristics and tuning guide","Praxa benchmarks","AI agent evaluation","reproducible benchmarks"]
---

~~~mermaid
flowchart LR
  Token["Token acquisition"] --> Request["Mission request"]
  Request --> Retry["Bounded retry"]
  Request --> SSE["SSE stream"]
  Request --> Fanout["Mission fan-out"]
  Budget["Resource budget"] --> Request
  Retry --> Outcome["Observed run outcome"]
  SSE --> Outcome
  Fanout --> Outcome
~~~
