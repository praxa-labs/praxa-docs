...
The source-ready hosted plane derives the tenant and actor from a reauthorized `praxa_sk_*` key.
...
Source and tests do not prove deployed isolation. Activation requires the additive migration, full
Worker deployment, exact-revision receipt, and authenticated positive, revoked-key, wrong-scope,
two-tenant, idempotency, export, deletion, and degraded-provider canaries.
