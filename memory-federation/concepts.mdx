...
| Provider ID | Source model preserved | Source-ready adapter boundary |
|---|---|---|
...
| `letta` | Core memory blocks and recent messages as different kinds | Inject a Letta client. Blocks map to `pinned_context`; messages map to `message`. The candidate does not claim Letta archival retrieval. |
...
- A provider timeout produces a partial-degradation result. If every source fails, the result is
