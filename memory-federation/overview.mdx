...
<Warning>
  **Current status:** the `@praxa/sdk/memory` adapters and CLI setup commands are published in
  ...
  `/v1/memory/*` candidate plane is deployed from Praxa AI revision
  `8bc8705522462c41105917f4f8f97e6f4b69f1c1` and its additive migration is proven live. Every
  route fails closed at the unauthenticated gateway boundary, but authenticated tenant, scope,
