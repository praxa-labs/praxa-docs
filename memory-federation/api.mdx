...
description: "Review the isolated /v1/memory candidate-plane routes and their source-ready, not-deployed status."
---

<Warning>
  The routes on this page are source-ready contracts. They are not deployed, migration-proven, or
  authenticated-canary proven. A `401` from `api.praxa.io` proves only the shared gateway
  authentication boundary; it does not prove that a named Memory route is available.
</Warning>
...
The portable-envelope schema is frozen and aligned across the runtime and `@praxa/sdk/memory`
source candidates. This page intentionally omits copy-paste hosted requests until the deployed
